...
def main():
    ...
    while running:
        ...
        if len(pf.particles) > 0:
            ...
            # Emergency Unlock (Sudden movement)
            ...
            # --- FIX 1: Prevent Wandering ---
            # Added "or not current_path". If path is empty, REPLAN IMMEDIATELY.
            if mcts_cooldown == 0 or emergency_unlock or not current_path:
                ...
                if new_path: 
                    # --- FIX 2: Prevent 180 U-Turns (The Stutter Step) ---
                    # If the first node is too close (or behind us), skip it.
                    if len(new_path) > 0 and np.linalg.norm(hunter.pos - new_path[0]) < 30:
                        new_path.pop(0)
            ...
            # PATROL MODE (Only runs if NO PARTICLES)
            if not current_path:
        ...
        if current_path:
            ...
            # --- FIX 3: Smoother Waypoint Switching ---
            # If we are close to the node, switch to the next one immediately
        ...
        # HUD (Simple Status)
        status_color = (0, 255, 100) if len(pf.particles) > 0 else (255, 150, 0)
        status_text = "TRACKING" if len(pf.particles) > 0 else "SEARCHING"
        pygame.draw.rect(screen, (10, 20, 40), (5, 5, 200, 30)) 
        pygame.draw.rect(screen, status_color, (5, 5, 200, 30), 1)
        font = pygame.font.SysFont("monospace", 16, bold=True)
        screen.blit(font.render(f"STATUS: {status_text}", True, status_color), (15, 10))
