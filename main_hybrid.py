...
class ParticleFilter:
    ...
    def update(self, observation, intruder_vel):
        self.frame_timer += 1 # Count frames
        
        if observation is not None:
            # --- CASE A: WE SEE THE TARGET ---
            # FIX: Only destroy/create particles every 5th frame (Save CPU!)
...
def main():
    ...
    mcts_interval = 10 
    # ... setup ...
    clear_shot_timer = 0  # <--- NEW VARIABLE
    # Timer variables
    ...
    while running:
        ...
        # --- STEP 4: HUNTER AI (STABILIZED HYBRID) ---
        ...
        # 2. CHECK BLOCKAGE (Shoulder Check)
        is_blocked = False
        if target_pos is not None:
            offset = 15 # Half width of drone
            # Check 3 Rays: Center, Left Edge, Right Edge
            rays = [
                (hunter.pos, target_pos),
                (hunter.pos + np.array([-offset, 0]), target_pos + np.array([-offset, 0])),
                (hunter.pos + np.array([offset, 0]), target_pos + np.array([offset, 0]))
            ]
            
            for start, end in rays:
                for obs in walls:
                    if obs.rect.clipline(start, end):
                        is_blocked = True
                        break
                if is_blocked: break
        
        # 3. STABILITY TIMER (The Fix)
        if not is_blocked and target_pos is not None:
            clear_shot_timer += 1
        else:
            clear_shot_timer = 0
        ...
        # CONDITION: Path must be clear for 20 frames (~0.3s) BEFORE switching
        # This prevents the "Flicker" near corners.
        if clear_shot_timer > 20:
             # --- BRAIN: MCTS (Sniper) ---
             if frame_count % mcts_interval == 0:
                 current_action = pomcp_hybrid.run_mcts(hunter, pf.particles, walls)
             steering = current_action 
             
             # Debug: Thick RED line means MCTS is fully engaged
             pygame.draw.line(screen, (255, 0, 0), hunter.rect.center, hunter.pos + steering*100, 4)

        else:
             # --- BRAIN: A* (Pilot) ---
             # Set Patrol Goal if needed
             if target_pos is None and not current_path:
                  target_pos = np.array([random.uniform(50, WIDTH-50), random.uniform(50, HEIGHT-50)])

             # Update A* Path
             path_timer += 1
             if target_pos is not None and path_timer > 30:
                 path_timer = 0
                 new_path = pathfinder.find_path(hunter.pos, target_pos)
                 if new_path: current_path = new_path

             # Follow Path
             if current_path:
                 node = current_path[0]
                 if np.linalg.norm(hunter.pos - node) < 40:
                     current_path.pop(0)
                     if current_path: node = current_path[0]
                 
                 desired = node - hunter.pos
                 if np.linalg.norm(desired) > 0:
                     desired_vel = (desired / np.linalg.norm(desired)) * 4.0
                     steering = desired_vel - hunter.vel
            
             # Debug: CYAN line means A* is flying
             if len(current_path) > 1:
                 pygame.draw.lines(screen, (0, 255, 255), False, current_path, 2)
        
        # 5. WALL SAFETY FORCE (Always Active)
        for obs in walls:
