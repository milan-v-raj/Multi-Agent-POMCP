...
# --- STABILIZATION VARIABLES ---
    ...
    last_belief_mean = None # <--- NEW: Track where we thought the enemy was
    mcts_cooldown = 0
    running = True
    while running:
        ...
        if len(pf.particles) > 0:
            ...
            if last_belief_mean is not None:
                # Compare Current Belief vs Old Belief (Did the ENEMY jump?)
                belief_shift = np.linalg.norm(current_belief_mean - last_belief_mean)
                if belief_shift > 100: # If enemy belief center moved > 100px instantly
                    emergency_unlock = True
                    mcts_cooldown = 0 

            # --- RUN MCTS ---
            # Only run if cooldown is done OR we have an emergency
            if mcts_cooldown == 0 or emergency_unlock:
                ...
                if new_path: 
                    ...
                    # Update the "Anchor" for the next check
                    last_belief_mean = current_belief_mean 
                    
                    # LOCK IT IN: 0.5 seconds (30 frames) is enough
                    mcts_cooldown = 30
