...
def main():
    ...
    # NEW: Initialize the Particle Filter
    ...
    while running:
        ...
        # C. HUNTER (Blue) - GREEDY BASELINE
        if can_see:
            # If we see it, CHASE IT!
            hunter.move_towards(intruder.pos)
            # Feed real data to Particle Filter
            observation = intruder.pos
        else:
            # If hidden, BRAKE (Baseline failure behavior)
            hunter.vel *= 0.9
            # Feed "None" to Particle Filter
            observation = None
            
        # D. BELIEF UPDATE
        pf.predict() # Move ghosts forward
        pf.update(observation, intruder.vel) # Correct ghosts

        # --- 2. UPDATE PHYSICS & CHECK COLLISIONS FIRST ---
        hunter.update()
        hunter.check_collision(walls)
        
        intruder.update()
        # Capture if the intruder hit a wall!
        intruder_hit_wall = intruder.check_collision(walls)

        # --- 1. AI LOGIC (After Physics) ---
        
        # Pass the collision signal to the wander function
