...
def draw_hud(screen, mode, gate_active):
    pygame.draw.rect(screen, UI_PANEL_COLOR, (10, 10, 260, 90), border_radius=5)
    pygame.draw.rect(screen, WALL_OUTLINE, (10, 10, 260, 90), 2, border_radius=5)
    ...
    text_inst = font_small.render("[M] Toggle AI | [R] Reset | [N] New Map", True, UI_TEXT_COLOR)
...
def main():
    ...
    pygame.display.set_caption("A/B Test: Heuristic vs Deep-POMCP (Procedural)")
    clock = pygame.time.Clock()

    # --- PROCEDURAL MAP GENERATOR ---
    def generate_map():
        # Spawn Switch anywhere on the left half
        sx = random.randint(50, WIDTH // 2 - 100)
        sy = random.randint(50, HEIGHT - 100)
        # Spawn Gate gap anywhere along the right wall
        gy = random.randint(50, HEIGHT - 170)
        
        # Calculate the wait position right outside the new gate
        wait_pos = np.array([WIDTH - 260.0, gy + 60.0])
        return np.array([sx, sy]), gy, wait_pos

    # Initial Map
    switch_pos, gate_y, gate_wait_pos = generate_map()
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_m:
                    ...
                    hunter1, hunter2, target, pf = reset_actors()
                    current_action1, current_action2 = np.array([0.0, 0.0]), np.array([0.0, 0.0])
                if event.key == pygame.K_r:
                    hunter1, hunter2, target, pf = reset_actors()
                    current_action1, current_action2 = np.array([0.0, 0.0]), np.array([0.0, 0.0])
                if event.key == pygame.K_n: # NEW MAP TRIGGER!
                    switch_pos, gate_y, gate_wait_pos = generate_map()
                    hunter1, hunter2, target, pf = reset_actors()
                    current_action1, current_action2 = np.array([0.0, 0.0]), np.array([0.0, 0.0])

        # --- DYNAMIC ARENA REBUILD ---
        switch_rect = pygame.Rect(switch_pos[0]-30, switch_pos[1]-30, 60, 60)
        gate_rect = pygame.Rect(WIDTH - 200, gate_y, 10, 120)
        gate_active = not (switch_rect.colliderect(hunter1.rect) or switch_rect.colliderect(hunter2.rect))
        
        current_arena = Arena()
        current_arena.walls.empty() 
        # Outer boundaries
        current_arena.walls.add(Obstacle(0, 0, WIDTH, 10))
        current_arena.walls.add(Obstacle(0, HEIGHT-10, WIDTH, 10))
        current_arena.walls.add(Obstacle(0, 0, 10, HEIGHT))
        current_arena.walls.add(Obstacle(WIDTH-10, 0, 10, HEIGHT))
        
        # Dynamic Target Room Walls
        current_arena.walls.add(Obstacle(WIDTH - 200, 10, 10, gate_y - 10)) # Top segment
        current_arena.walls.add(Obstacle(WIDTH - 200, gate_y + 120, 10, HEIGHT - (gate_y + 120) - 10)) # Bottom segment
        
        if gate_active:
            current_arena.walls.add(Obstacle(gate_rect.x, gate_rect.y, gate_rect.width, gate_rect.height))
        ...
        if frame_count % mcts_interval == 0:
            if mode == "HEURISTIC":
                ...
                # PASSING THE DYNAMIC POSITIONS TO THE BRAIN
                current_action1 = get_deep_action(hunter1, hunter2.pos, target.pos, spread, current_arena, switch_pos, gate_wait_pos)
        ...
        if (frame_count + 15) % mcts_interval == 0:
            if mode == "HEURISTIC":
                ...
                _, spread = pf.get_belief_center_and_spread()
                current_action2 = get_deep_action(hunter2, hunter1.pos, target.pos, spread, current_arena, switch_pos, gate_wait_pos)
        ...
        if np.linalg.norm(hunter1.pos - target.pos) < 20 or np.linalg.norm(hunter2.pos - target.pos) < 20:
            ...
            switch_pos, gate_y, gate_wait_pos = generate_map() # Auto-randomize on win
            hunter1, hunter2, target, pf = reset_actors()
            current_action1, current_action2 = np.array([0.0, 0.0]), np.array([0.0, 0.0])
