...
class LightweightState:
    ...
    def step(self, action_idx):
        ...
        # Check collisions (using the passed obstacles list)
        hit_wall = False
        if not (0 < next_pos[0] < WIDTH and 0 < next_pos[1] < HEIGHT):
            hit_wall = True
        ...
        if hit_wall:
            # Penalize but allow simulation to continue (Soft Wall)
            self.h_pos = next_pos 
            return -10.0, False 
...
def run_mcts(hunter, particles, obstacles):
    # --- 1. GHOST PHYSICS ENABLED ---
    # We pass an empty list for obstacles. 
    # MCTS will ignore walls and plan aggressive straight lines.
    sim_obstacles = [] 
    ...
    for _ in range(NUM_SIMULATIONS):
        ...
        # Create state with Ghost Obstacles (Empty)
        state = LightweightState(hunter.pos, hunter.vel, target_pos_guess, target_vel_guess, sim_obstacles)
