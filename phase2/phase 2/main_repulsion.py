    ...
    while running:
        ...
        if frame_count % mcts_interval == 0:
            if len(pf.particles) > 0:
                ...
                # Pass it into MCTS
                strategic_force1 = pomcp_beforeNN.run_mcts(hunter1, pf.particles.sprites(), walls, ally_target=ally_tgt)
        ...
        if (frame_count + 15) % mcts_interval == 0:
            if len(pf.particles) > 0:
                ...
                # Pass it into MCTS
                strategic_force2 = pomcp_beforeNN.run_mcts(hunter2, pf.particles.sprites(), walls, ally_target=ally_tgt)
                
                # ... (rest of hunter 2 logic)
        ...
        # --- DRONE-TO-DRONE SEPARATION (Boids Reflex) ---
        ...
        if len(pf.particles) > 0:
             status = "SWARM TRACKING"
             color = (0, 255, 100)
        else:
             status = "SWARM SEARCHING"
             color = (255, 150, 0)
